...
class Expense:
    ...
    def apply_to_ledger(self, group: 'Group'):
        """Create debts in group for each participant (except payer)"""
        shares = self.calculate_shares()
        for user, amount in shares.items():
            if user != self.payer and amount > 0:
                group.add_debt(user, self.payer, amount)
...
# ==================== GROUP (REFACTORED - DEBTS INLINED) ====================
class Group:
    def __init__(self, name: str, description: str, owner: User, is_friend_group: bool = False):
        ...
        self.debts: List[Debt] = []  # ← INLINED from DebtLedger
    ...
    def view_members(self) -> List[str]:
        ...
        return [f"{m.user.name} ({m.role})" for m in self.members]
    
    # ===== EXPENSE MANAGEMENT =====
    
    def add_expense(self, expense: Expense):
        """Add expense and apply debts to ledger"""
        self.expenses.append(expense)
        expense.apply_to_ledger(self)
    
    def view_expenses(self) -> List[str]:
        """Return list of expense details"""
        return [exp.get_details() for exp in self.expenses]
    
    # ===== DEBT MANAGEMENT (INLINED from DebtLedger) =====
    
    def add_debt(self, debtor: User, creditor: User, amount: Decimal) -> Debt:
        """Create and store debt record"""
        debt = Debt(debtor, creditor, amount)
        self.debts.append(debt)
        return debt
    
    def get_debt(self, debtor: User, creditor: User) -> Decimal:
        """Return total debt in one direction (sum of all ACTIVE debts)"""
        total = Decimal(0)
        for debt in self.debts:
            if (debt.debtor == debtor and debt.creditor == creditor 
                and debt.status == DebtStatus.ACTIVE):
                total += debt.amount
        return total
    
    def get_simplified_debts(self) -> Dict[Tuple[str, str], Decimal]:
        """Calculate net debts between all user pairs"""
        simplified = {}
        users = set()
        
        # Collect all users involved in ACTIVE debts
        for debt in self.debts:
            if debt.status == DebtStatus.ACTIVE:
                users.add(debt.debtor)
                users.add(debt.creditor)
        
        # For each pair, calculate net
        users_list = list(users)
        for i, user1 in enumerate(users_list):
            for user2 in users_list[i+1:]:
                debt_1_to_2 = self.get_debt(user1, user2)  # user1 → user2
                debt_2_to_1 = self.get_debt(user2, user1)  # user2 → user1
                net = debt_2_to_1 - debt_1_to_2
                
                if net > 0:
                    simplified[(user2.name, user1.name)] = net  # user2 owes user1
                elif net < 0:
                    simplified[(user1.name, user2.name)] = abs(net)  # user1 owes user2
        
        return simplified
    
    def settle_debt(self, debtor: User, creditor: User, amount: Decimal):
        """Mark debts as settled (decrease amount or mark SETTLED)"""
        for debt in self.debts:
            if (debt.debtor == debtor and debt.creditor == creditor 
                and debt.status == DebtStatus.ACTIVE):
...
class SplitSmartUI:
    ...
    def show_settle_debt_dialog(self, group, simplified):
        ...
        def settle():
            ...
            # Settle the debt using refactored Group method
            group.settle_debt(debtor, creditor, payment)
